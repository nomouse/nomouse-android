...
import com.tixa.lx.servant.common.base.PageListView.Status;
...
public abstract class AbsPageAdapter<T> extends AbsAdapter<T> {
	...
	public void notifyDataSetChanged(List<T> result) {
		if (result == null) {
			...
			// 列表为空时要显示空布局
			listView.currentStatus = Status.MAIN_EMPTY;
			...
			if (result.size() == 0 || result.size() < PAGE_SIZE) {
				...
				listView.currentStatus = Status.LIST_FULL;
			} else {
				listView.currentStatus = Status.LIST_MORE;
			}
